...
export function unwrapWithDefault(...args) {
  if (args.length === 1) {
    return createNamedFn('unwrapWithDefault.curry', result => unwrapWithDefault(result, args[0]))
  }
...
export function mapWithDefault(...args) {
  if (args.length === 2) {
    ...
    return createNamedFn('mapWithDefault.curry', result => mapWithDefault(result, mapper, defaultValue))
  }
...
export function filter(...args) {
  if (args.length === 2) {
    ...
    return createNamedFn('filter.curry', result => filter(result, predicate, failValue))
  }
